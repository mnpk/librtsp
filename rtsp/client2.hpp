...
class ClientBase {
 public:
  ...
  std::shared_ptr<Response> request(
      const std::string& request_type, const std::string& path = "/",
  ...
  std::shared_ptr<Response> request(
      const std::string& request_type, const std::string& path,
      std::ostream& content, const std::map<std::string, std::string>& header =
                                 std::map<std::string, std::string>()) {
    ...
    write_stream << request_type << " " << corrected_path << " RTSP/1.1\r\n";
    ...
    try {
      ...
      size_t bytes_transferred = boost::asio::read_until(
          *socket, response->content_buffer, "\r\n\r\n");

      size_t num_additional_bytes =
          response->content_buffer.size() - bytes_transferred;
      ...
      if (response->header.count("Content-Length") > 0) {
        boost::asio::read(*socket, response->content_buffer,
                          boost::asio::transfer_exactly(
                              stoull(response->header["Content-Length"]) -
 ...
 protected:
  ...
  void parse_response_header(std::shared_ptr<Response> response,
    ...
    if (version_end != std::string::npos) {
      ...
      response->status_code =
          line.substr(version_end + 1, line.size() - version_end - 2);
      ...
      while (param_end != std::string::npos) {
        ...
        response->header[line.substr(0, param_end)] =
...
typedef boost::asio::ip::tcp::socket RTSP;

template <>
class Client<RTSP> : public ClientBase<RTSP> {
 public:
  Client(const std::string& server_port_path)
      : ClientBase<RTSP>::ClientBase(server_port_path, 80) {
    socket = std::make_shared<RTSP>(asio_io_service);
  }
